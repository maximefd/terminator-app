...
- L'estimation **tient compte de la taille de grille** quand l'auteur en a choisi une, par classe (petite ≤ 21 emplacements, moyenne, grande ≥ 61). Mesuré : **l'effet de la taille s'inverse selon la demande** — pour des mots courts la petite grille gagne (2 mots courts : 100 % contre 77 %), pour des mots longs la grande (3 mots longs à lettre rare : 31 % contre 13 %). Deux autres signaux par format ont été réfutés avant celui-ci : le nombre d'emplacements de la bonne longueur ne prédit rien, et classer les layouts par charge de croisements n'apporte pas de gain distinguable du bruit.
- Limite assumée : l'estimation ignore le **layout précis** visé, seulement sa classe de taille. La vérification préalable existante (`check_must_words`) couvre déjà le cas « ce mot n'entre nulle part ».
