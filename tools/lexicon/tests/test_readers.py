...
from tools.lexicon.readers import read_dela, read_lexique, read_wiktionary, resolve_definition
...
def test_read_wiktionary_keeps_french_definitions(wiktionary_file):
...
def test_inflected_form_definition_includes_the_lemma_definition(wiktionary_file):
    wiktionary = read_wiktionary(wiktionary_file, wanted={"OUVRAGEAMES"})

    assert resolve_definition("OUVRAGEAMES", wiktionary) == (
        "Première personne du pluriel du passé simple de ouvrager. — Travailler avec soin."
    )
    assert resolve_definition("AABAM", wiktionary) is None


def test_long_definitions_are_shortened(tmp_path):
    from tools.lexicon.readers import MAX_DEFINITION_LENGTH, WiktionaryEntry

    long_text = "mot " * 200
    definition = resolve_definition("LONG", {"LONG": WiktionaryEntry(definition=long_text)})

    assert len(definition) <= MAX_DEFINITION_LENGTH + 1
