...
def load_user(user_id):
    """Indique à Flask-Login comment charger un utilisateur depuis un ID de session."""
    with app.app_context():
        # Utilisation de db.session.get() pour la récupération par clé primaire
...
class Utilisateur(db.Model, UserMixin):
    ...
    mots_personnels = db.relationship('MotPersonnel', backref='utilisateur', lazy=True)

class MotPersonnel(db.Model):
    id = db.Column(db.Integer, primary_key=True)
...
def initialize_demo_user_and_words():
    """Crée l'utilisateur de démo (demo@terminator.com) et ajoute des mots de démo."""
    demo_email = 'demo@terminator.com'
    
    with app.app_context():
        user = Utilisateur.query.filter_by(email=demo_email).first()
        if not user:
            user = Utilisateur(email=demo_email, password="password") 
            db.session.add(user)
            db.session.commit()
            logger.info(f"Utilisateur de démo '{demo_email}' créé.")
        
        # Ajout de mots de démo s'il n'en a pas
        if not user.mots_personnels:
            demo_words_data = [
            ...
            db.session.commit()
            logger.info(f"2 mots de démo personnels ajoutés pour l'utilisateur {user.id}.")
